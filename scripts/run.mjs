...
async function main() {
  ...
  if (values.help) {
    console.log('node scripts/run.mjs [--host native|browser] [--case DIRECTORY | --filter TEXT] [--reverse] [--online] [--binary PATH] [--report PATH]');
    return;
  }
  if (!['native', 'browser'].includes(values.host)) throw Error('host must be native or browser');
  ...
  try {
    for (const test of cases) {
      ...
      if (reason) {
        counts.SKIP++;
        results.push({ id: test.id, status: 'SKIP', reason });
        console.log(`SKIP  ${test.id} — ${reason}`);
        continue;
      }
      const request = { source: test.source, api: test.api };
      let actual;
      if (browser) actual = await browser.run(test, request);
      else {
